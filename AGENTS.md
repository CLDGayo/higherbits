...
**Graphify Integration:** Before asking any clarifying questions to the user about project state, context, or code location, ALWAYS run a quick search in `graphify` (e.g., via `/graphify query "<question>"`) to see what is currently available to your query. Only ask questions if the graphify query does not resolve the ambiguity.
...
Always utilize the `graphify` skill in every prompt, no exceptions.

## Roadmap Phase Documentation Rule

When documenting or drafting upcoming phases in the ultimate roadmap (or any phase plan), always include a `Recommended Agents/Skills (Non-Core):` line. This line should assign appropriate non-core specialist agents and helper skills (e.g., `vc-ui-ux-designer`, `vc-security`, `vc-predict`, `vc-scenario`, `modern-web-guidance`) to the phase to ensure the right tools are surfaced for future execution.
