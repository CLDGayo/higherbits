...
Produce a repeatable end-of-session summary so the user can clear context or start a new agent without losing continuity. The next agent should be able to pick up by reading this summary alone.

This is a **context-handoff artifact**, not a status report. The audience is a future instance of you (or another agent), not a stakeholder.

## When to invoke

User says: "session handoff", "wrap up session", "hand off", "handoff summary", "let's wrap up", "summarize before I clear", or any near-equivalent. Also invoke proactively if the user says they're about to clear context without having run it yet.
...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `process/general-plans/active/` or `process/features/*/active/` if a plan was mentioned).
   - Tasks state — any in-progress or pending tasks on the checklist.
   - Background processes you started (e.g. dev servers, watcher scripts) — task IDs/shell IDs are load-bearing for the next agent.
   - Files created or modified this session — you know what you touched; don't grep to re-discover.
   - Repository context/memory files written or updated (e.g., `process/context/all-context.md` or `.claude/memory/`).
...
- Context/Memory files touched: `<paths>` (if any)

## Running state
- Background processes: <task IDs / shell IDs + what they are + how to kill> — or "none"
...
5. **No emojis, no hype, no "great job" summaries.** Terse and concrete — paths, commands, task IDs, decisions. Match the tone of a seasoned engineer handing off at end-of-shift.
6. **Background process IDs are critical.** If you started any background tasks, their IDs must appear in "Running state" with the kill command — the next agent cannot find them otherwise.
