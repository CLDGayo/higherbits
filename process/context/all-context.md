...
Every agent (Orchestrator, Claude, Codex, Antigravity, and ALL RIPER-5 subagents) MUST execute a `graphify` query (`graphify query "<topic/keywords>"`) as Step 0 before loading context files, writing plans, or taking action. After modifying code, always run `graphify update .`.
...
- **Deployment (higherbits.dev) — gayo-vps, NEVER Vercel.** higherbits.dev is self-hosted behind nginx on the user's own VPS (gayo-vps = `ssh root@72.62.196.231`, key auth). **CORRECTED 2026-07-16 (discovered during the `claymorphism-3d-redesign` deploy), RE-CONFIRMED 2026-07-18 (`claymorphism-reference-parity` deploy, SHA `bfa2573`):** the live app runs from `/home/higherbits/htdocs/higherbits.dev` (user `higherbits`), served by pm2 app `higherbits.dev` (`pnpm start` → `next start`, port 3005, nginx proxy unchanged). This supersedes the prior `/home/cozy/htdocs/higherbits.dev` + pm2 app `higherbits` claim (verified 2026-07-13) — that path/user is the STALE/DEAD copy (still present on disk but not the live one). Deploy procedure (same shape, path/user/pm2-name updated): push to `origin main` (github.com/CLDGayo/higherbits), then `su - higherbits -c "cd ~/htdocs/higherbits.dev && git pull --ff-only origin main && corepack pnpm install --no-frozen-lockfile && NODE_OPTIONS=\"--max-old-space-size=3072\" corepack pnpm --filter web build && pm2 restart higherbits.dev"` — always `su - <app-user>`, never `sudo -u <app-user>` (HOME pollution breaks corepack). CRITICAL: the NODE_OPTIONS heap bound is required — unbounded build gets OOM-killed (exit 137); on a flaky SSH session, prefer a detached/`nohup`-backed build so a dropped connection doesn't kill the build. Pushing to GitHub alone does NOT deploy. `apps/web/vercel.json` is an upstream 21st.dev leftover — its two crons (`/api/cron/gen-usage-embeddings` hourly, `/api/subscription/stripe-cron` monthly) never run and need VPS crontab or n8n equivalents.
  ...
  **2026-07-18 deploy learnings (`claymorphism-reference-parity` program close, 5 attempts to green — durable process fixes, not one-off notes):**
  1. **The VPS can no longer run `next build`'s type-check step within its RAM budget** — even the documented 3072MB heap bound OOM-kills (exit 137) on the current app size during "Linting and checking validity of types." Durable fix: verify `tsc --noEmit` + lint are green **locally** on the exact committed tree first (a disposable scratch worktree checkout, not the dirty working tree), then run the VPS build with `SKIP_BUILD_VALIDATION=true` appended to the build command. Never set `SKIP_BUILD_VALIDATION=true` without that prior local-green confirmation — it silences real compile errors, not just the OOM.
  2. **The VPS working tree accumulates drift between deploys**: (a) untracked files land there via the user's separate `sync-to-vps.sh` rsync workflow and will conflict with `git pull --ff-only`; (b) live hot-fixes sometimes get hand-edited directly on tracked files on the VPS. Before every deploy pull: back up untracked conflicts (`mv` aside to a dated `~/untracked-conflict-backup-*/` dir) and verify tracked-file dirty diffs byte-for-byte against the incoming commit (or `git stash` with a dated message, e.g. `pre-deploy-<ddmm>-live-edits`) BEFORE overwriting — do not blind-overwrite VPS-side edits.
  3. **Never commit build cache directories** (e.g. `apps/web/.next-prod/`) — a webpack pack file over GitHub's 100MB limit blocked a push during this deploy and required a `git filter-branch --index-filter` history rewrite of 20 unpushed commits to remove. `.next-prod/` is now gitignored at both root and `apps/web/`. If a future push is blocked by a large-file rejection, check `git ls-files | xargs du` on the diff range before assuming a rewrite is needed.
  4. **Module-scope SDK construction in Next.js API routes breaks build-time page-data collection** if the required env var isn't set in the build environment (e.g. `new Stripe(...)` executed at import time). Repo-standard guard pattern: check `paymentsNotConfigured()` → return 503, and `await import("stripe")` dynamically inside the request handler, not at module scope.

  **Before your next deploy, still re-verify the live path/user/pm2-name with `pm2 list` and `ls /home/*/htdocs/`** if more than a few weeks have passed — this has drifted before without a corresponding harness update.

## Scan Metadata

- Generated: 2026-06-24 (vc-setup, Flow A); updated 2026-06-25 for Phases 10/11/12/13 VERIFIED COMPLETE; updated 2026-06-28 for Phase 1 CODE DONE (billing/auth) and Phase 2 VERIFIED COMPLETE (registry pruned 139→5, validate-registry.mjs live, ops hardened, packages/db schema extended, Category now open string); updated 2026-07-01 for the security-hardening pass (commit `9a3593d`) — submitComponent rate limit, webhook payment_status allow-list, userId-keyed rate limits (checkout/connect/dashboard/return), Connect account reuse; test suite now 60 tests across 15 files (54 pass / 6 pre-existing baseline failures tracked in backlog); updated 2026-07-04 for Phase 17 multi-demos (commit `a10aa41`) — Demos frontmatter field, per-demo source heading convention, validate-registry Demos validation + security constraints, ComponentPayload Demos field, PreviewTabs demo pill selector, stripDemoPaywall, SoftButtonAdvanced curated variant, Git LFS .gitattributes, ops/copy-demo-video.mjs; test suite now 71 tests across 17 files (65 pass / 6 baseline failures unchanged); updated 2026-07-04 for Phase 18 R2 Migration — gracefully absent R2 integration, ops/r2-client.mjs lazy-init, registry.ts hydration fallback, NEXT_PUBLIC_CDN_URL demo video prefixing, ops/github-ingest.mjs R2 upload hook; updated 2026-07-05 for Phase 19 Templates & Themes (commit `7ad466e`) — Content_Type extended to template|theme, Palette_Tokens frontmatter field + validation, RegistryEntry contentType/installSnippet/paletteTokens fields, theme source-guard opt-out in parseRawEntry, new ThemeDetail server component, 4 new curated seed files (registry now 9 curated files total), ops/upload-seed-entries.mjs R2 wrapper with IsPro-skip guard; the prior 6 baseline vitest failures were already resolved by Phase 18 — test suite now 87 tests across 19 files, all passing; updated 2026-07-06 for cozy-21st-mirror Phase 3 Landing Page (VERIFIED) — SearchIsland wired into Hero, premium-token pass on Hero/FeaturedStrip/TrendingStrip/ValueProp, 5 new landing-page tests, established async-Server-Component await-then-render RTL pattern and fake-timer+act() debounce-assertion pattern; test suite now 102 tests across 25 files, all passing; updated 2026-07-09 for 21st-promotion Phase 1 Backend & Schema Merge (VERIFIED) — 21st-dev's 40-model Prisma schema merged into packages/db (users renamed to local_users, 21 FK relations preserved), first Prisma tooling + first test script in packages/db, new prisma.ts subpath export, Clerk webhook gained user.created/updated/deleted branches (test suite 3→8), live Supabase Postgres migrated via `prisma db push`, zero live wiring of 21st-dev billing/search/webhook code confirmed by grep security gate, 21st-dev/apps/backend registered as a workspace member; updated 2026-07-09 for 21st-promotion Phase 2 Frontend & UI Migration (VERIFIED) — Hybrid architecture decision locked (primitives + polish only, no marketplace port, dormant Prisma tables stay unwired), 12 UI primitives ported into new apps/web/components/ui/ directory and token-remapped to root's authoritative scheme, 12 new jsdom smoke-render tests (apps/web test suite 102→123 across 25→27 files, all passing), 7 new lightweight Radix deps, packages/ui confirmed untouched, packages/ui/src/cozy/* attribution gap carried to backlog; updated 2026-07-13 for HigherBits.dev Rebrand (VERIFIED) — 21st.dev strings replaced, logo fixed, tokens updated to cozy claymorphism, studio access fixes, Lemon Squeezy boilerplate ready, Supabase "users" bucket added; updated 2026-07-17 for `claymorphism-reference-parity` Phase 1 (VERIFIED, commit `f109b3f`) — 8 clay assets converted to real-alpha WebP via a new `ops/gemini-asset-chroma-key.mjs` script, `sharp` added as a root devDependency (ops-time-only), `.clay-card-icon`/`.clay-card-illustration` CSS + `--accent-lavender`/`--accent-cream` pastel tokens added; test suite re-baselined 87/19→37/13 (this file's prior 87/19 figure was itself already stale — see `process/context/tests/all-tests.md` for the reconciled count); a11y pre-existing-failure baseline corrected from 5 to 8 (added 2× link-name + 1× text-primary contrast, previously undercounted); updated 2026-07-18 for `claymorphism-reference-parity` program close (Phases 2-4 VERIFIED, deploy EXECUTED and CONFIRMED LIVE, SHA `bfa2573`) — deploy learnings folded into §Deployment (VPS type-check OOM workaround via `SKIP_BUILD_VALIDATION=true` after local tsc confirmation, VPS working-tree drift/backup discipline, `.next-prod/` build-cache now gitignored after a history-rewrite removal, module-scope-SDK-construction guard pattern), program task folder archived to `completed/`.; updated 2026-07-26 for `supabase-interconnect`
  Phase 03 (Scheduler + seed) CODE-COMPLETE and locally verified — embeddings backfill cron
  extended additively (dry-run + batch cap), crontab install artifact + seed SQL delivered
...
- **Claymorphism + 3D Pastel Soft UI program — COMPLETE, deployed and verified (updated 16-07-26).** `apps/web/app/globals.css` has `--clay-shadow-light/dark/outer`, `--clay-depth-sm/md/lg`, `--clay-pressed`, `--accent-yellow(-foreground)`, and a `.clay-surface` utility (both `:root` and `.dark`), added additively by Phase 1. Phase 3 (Component Library) built 5 components in `apps/web/components/ui/`: `clay-card.tsx`, `clay-input.tsx`, `clay-pill-button.tsx`, `clay-pill-bar-chart.tsx`, `clay-donut-chart.tsx` (thin recharts wrappers, no new charting dependency). Phase 4 (Page Assembly & Layout) wired the full Clay set into `hero-section.tsx` and `public-dashboard/page.client.tsx`. Phase 5 (Refinement, Animation & Deployment, FINAL PHASE, EVL-confirmed 16-07-26) added an opt-in `.clay-interactive` press-down modifier (ships defined-but-unused — no genuinely interactive `.clay-surface` consumer exists in current routes), a `prefers-reduced-motion` guard covering both `.clay-interactive` and `ClayPillButton`'s existing transition, mobile-viewport responsive shadow/padding scaling, and a new `apps/web/e2e/visual-evidence.spec.ts` Playwright spec capturing 8 screenshots (hero + dashboard × light/dark × desktop/375px) into the program's task folder — closing the program's long-standing visual-parity Agent-Probe debt. **All gates green** (build/tsc/vitest 29/29 across 11 files/a11y 13-pass-5-known-gap). **Deploy EXECUTED and VERIFIED 16-07-26** via the gayo-vps pm2 procedure (explicit user authorization) — live at `https://higherbits.dev`, BUILD_ID `M4KO6WWsu6STGRpJWXKTI`; this deploy discovered and corrected a stale deploy-path claim in §Deployment below (live app moved `/home/cozy` → `/home/higherbits`, pm2 app renamed `higherbits.dev`). Task folder archived: `process/features/claymorphism-3d-redesign/completed/claymorphism-3d-redesign_14-07-26/` — see its `claymorphism-3d-redesign-umbrella_PLAN_14-07-26.md` `## Program Closeout` for full Definition-of-Done scoring and the Phase 5 report's `## Deploy Confirmation (16-07-26)` for the deploy narrative. Remaining items are optional and non-blocking: an optional ~$0.31 Gemini seed-batch run to populate `apps/web/public/clay/{icons,illustrations,textures}/` (currently `.gitkeep`-only; components render fine without it), and the pre-existing muted-foreground contrast backlog note.
...
- **Gemini asset-generation pipeline now exists (added 15-07-26, `claymorphism-3d-redesign` Phase 2).** `ops/gemini-asset-gen.mjs` (+ `ops/gemini-prompts.mjs` static prompt-template data module) is an ops-time-only Node script that calls the Gemini image-generation API, gated by shell-exported `GEMINI_API_KEY` / `GEMINI_IMAGE_MODEL` env vars only (never a `.env*` file). `apps/web/public/clay/{icons,illustrations,textures}/` exist but are empty pending a user-approved live seed-batch run — see the Phase 2 report for the opt-in command and cost estimate. **SUPERSEDED 17-07-26 (see next bullet):** the seed-batch run has since happened and the 8 assets are no longer JPG/empty.
- **Clay assets are now real-alpha WebP; new `sharp` devDependency + chroma-key ops script (`claymorphism-reference-parity` Phase 1, VERIFIED 17-07-26, commit `f109b3f`).** The Gemini seed batch (commit `3200f85`) populated the 8 assets under `apps/web/public/clay/{icons,illustrations,textures}/` as JPGs with a baked fake-transparency checkerboard. Phase 1 of the `claymorphism-reference-parity` program converted all 8 to real-alpha WebP via a new one-time ops script, `ops/gemini-asset-chroma-key.mjs` (saturation/lightness pixel classification + soft alpha ramp + per-asset override table + connected-component despeckle; unit-tested at `ops/__tests__/gemini-asset-chroma-key.test.mjs`, 10 tests). This required adding `sharp` (`^0.34.0`) to root `package.json` **devDependencies only** — ops-time-only, never imported by app code, never bundled into the `apps/web` production build. `.clay-card-icon`/`.clay-card-illustration` CSS classes and new `--accent-lavender`/`--accent-cream` pastel token pairs (light+dark, both ≥5.6:1 AA) were also added to `apps/web/app/globals.css`. In-page `<ClayCard>` rendering of these assets is deferred to Phase 3/4 of the same program (no consumer wired yet). See `process/features/claymorphism-reference-parity/active/claymorphism-reference-parity_16-07-26/phase-01-assets-css-foundation_REPORT_16-07-26.md`.
- **`claymorphism-reference-parity` program — ALL 4 PHASES VERIFIED, program VERIFIED WITH_GAPS, DEPLOYED AND LIVE (deploy executed + confirmed 18-07-26, SHA `bfa2573`; program archived to `completed/`).** Follow-up to `claymorphism-3d-redesign`: closed the gap between the shipped Clay token/component system and the deployed app actually looking like the pastel reference image. Phase 1 (assets/CSS foundation, `f109b3f`) chroma-keyed 8 Gemini JPGs to real-alpha WebP + `.clay-card-icon`/`.clay-card-illustration` CSS + lavender/cream tokens. Phase 2 (chart fixes, `d2a183d`) gave `ClayPillBarChart` per-category colors and `ClayDonutChart` a legend with percentages. Phase 3 (sidebar/tiles/mascot, `711a937`) added the sidebar lavender active-pill + Go-Premium card, expanded the dashboard to a 5-tile pastel grid, and integrated the hero mascot. Phase 4 (typography/QA/deploy, `06c35e5`, EVL-closed 18-07-26) applied `font-cozy` (Quicksand) to a locked element list (hero h1/h2/nav-brand, dashboard h1 + 5 stat-tile numbers, sidebar Go-Premium label), extended `apps/web/e2e/visual-evidence.spec.ts` to a 3-route/12-screenshot matrix (11/12 captured; 1 miss is a foreign missing Supabase RPC), and drafted a gayo-vps deploy request. Test baseline moved 37/13 → **48 tests / 15 files** (new `apps/web/app/__tests__/font-cozy-sweep.test.tsx`). 9 of the SPEC's 10 acceptance criteria are fully MET; the 10th (AC9, "existing build/tsc/test gates stay green") is partially unmet on its build/tsc leg on the pre-deploy *working tree* — 100% foreign-attributed (0 errors in any program file; see `foreign-build-tsc-red_NOTE_18-07-26.md`); this residual turned out to be working-tree-only — the actually **committed** `main` tsc'd clean (2 spurious errors from a missing generated `next-env.d.ts` only) once verified in an isolated scratch worktree during the deploy. A11y baseline moved 8→6 pre-existing foreign color-contrast fails (net improvement, 0 NEW introduced by this program) — see `process/context/tests/all-tests.md`. **Deploy EXECUTED and VERIFIED 18-07-26** (5 attempts to green; blockers found+fixed: a `.next-prod/` build-cache dir >100MB required a `git filter-branch` history rewrite of 20 unpushed commits before push would succeed; VPS pull needed untracked-file + dirty-tracked-file reconciliation backed up on the VPS before overwrite; VPS build OOM-killed on `next build`'s type-check step even at the documented 3072MB heap bound, worked around with `SKIP_BUILD_VALIDATION=true` after confirming tsc green locally on the identical committed tree; a module-scope Stripe SDK construction in a foreign support-us route broke build-time page-data collection, fixed with the repo-standard `paymentsNotConfigured()` + dynamic-import guard pattern) — see `process/context/all-context.md` §Deployment for the durable process learnings extracted from this deploy. Live at `https://higherbits.dev`; pm2 app `higherbits.dev` restarted and online. Task folder archived: `process/features/claymorphism-reference-parity/completed/claymorphism-reference-parity_16-07-26/` — see its umbrella plan's `## Program Closeout` for full Definition-of-Done scoring and the `## Deploy Confirmation (18-07-26)` addendum for the deploy narrative.
- **`supabase-interconnect` program — Phase 01 (Grant/RLS repair) CODE-COMPLETE, NOT LIVE-VERIFIED
  (26-07-26).** A 6-phase program fixing/connecting existing Supabase surface (no new product
  features) is in progress; task folder `process/features/supabase-interconnect/active/supabase-interconnect_25-07-26/`.
  Phase 01 authored 30 SQL statements across `supabase/{views,restore-authenticated-grants,admin-functions}.sql`
  restoring `authenticated` grants for `demo_bookmarks`, `prompt_rules`, `feedback`,
  `demo_hunt_leaderboard`/`demo_hunt_scores`/`demo_hunt_votes`, `component_dependencies_graph_view_v3`/
  `component_dependencies_closure`, `mv_component_analytics`, and a new `public.public_profiles` view
  (11 safe columns, `security_invoker=off`, used to fix 3 views that were silently returning 0 rows
  cross-user under `security_invoker=on` own-row RLS). **Security fix included:** the live table-level
  `GRANT SELECT, UPDATE ON public.users TO authenticated` allowed self-escalation (a user could set
  their own `is_admin=true`) — replaced with a `REVOKE UPDATE` + a 13-column-scoped `GRANT UPDATE`
  excluding `is_admin`/`email`/`ref`/`paypal_email`/`stripe_id`/etc. **Nothing has been applied to the
  live database yet** — a mandatory user-approval hard stop (Step C3) gates apply+live-verification.
  Genuine new gap found and NOT fixed: `public.templates` has a live `authenticated` INSERT call site
  (`publish-template-form.tsx:119`) with no tracked write grant — backlog note written. Postgres grant
  semantics learned (durable, applies to future Supabase SQL work): a column-scoped `GRANT UPDATE (cols)`
  does NOT remove a pre-existing table-level `GRANT UPDATE` — REVOKE must precede it; `EXECUTE` on new
  ...
  database. **Correction (26-07-26, Phase 03 close): "Phase 3's cron work depends on that eventual
  live apply" (the sentence that stood here) was wrong** — Phase 2's own Exit Gate and Phase 3's own
  Entry Gate/validate-contract both accept **scratch-verified** functions (the pglite-harness proof
  above) as sufficient to satisfy the F3 dependency; a live Supabase apply of Phase 2 was never a
  hard requirement to start Phase 3. An orchestrator ruling on 26-07-26 corrected the umbrella plan's
  stricter closeout narrative to match the gate text it was supposed to summarize (see the umbrella
  plan's Phase Sequence table note and Phase 03's own "Orchestrator ruling" section for the full
  reasoning) — Phase 3 proceeded on Phase 2's scratch-verified functions and completed with all
  gates green, vindicating the ruling. **Test baseline corrected
  ...
  by Phase 01 below — see `process/context/tests/all-tests.md` for the full history. **Phase 03
  (Scheduler + seed) CODE-COMPLETE AND LOCALLY VERIFIED, NOT ARCHIVED — stays in `active/` pending
  operator action (26-07-26).** Extended the existing
  `apps/web/app/api/cron/gen-usage-embeddings/route.ts` additively (no new standalone script, per a
  locked plan decision): `?dryRun=true` short-circuit before any `functions.invoke` call, and an
  `EMBEDDING_CRON_BATCH_CAP`-driven cap (default 20/run) applied unconditionally before the
  per-item loop — an intentional, safe default-behavior change since this route has zero live
  traffic today (its `vercel.json` cron entry never fires; the app runs on gayo-vps pm2). Delivered
  `ops/README-embedding-cron.md` (the crontab install artifact, explicitly operator-only — no agent
  performs any step in it) and `supabase/seed-embedding-verification.sql` (idempotent, FK-safe via
  a `SELECT`-driven subquery, correctly a clean no-op on an empty `users` table; authored, reviewed,
  **not applied to any database**). Test baseline moved 62/17 → **73 tests / 18 files, ALL 73
  PASSING** (11 new tests for the route, zero regressions); `tsc --noEmit` re-confirmed at 0 errors
  (the foreign `add-registry-modal.tsx` error the Phase 02 note above still lists was fixed by
  further concurrent work). Nothing installed on the VPS, nothing applied to the live database — both
