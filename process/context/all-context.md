...
- **`claymorphism-reference-parity` program — ALL 4 PHASES VERIFIED, program VERIFIED WITH_GAPS, kept in `active/` pending user-gated deploy (18-07-26).** Follow-up to `claymorphism-3d-redesign`: closed the gap between the shipped Clay token/component system and the deployed app actually looking like the pastel reference image. Phase 1 (assets/CSS foundation, `f109b3f`) chroma-keyed 8 Gemini JPGs to real-alpha WebP + `.clay-card-icon`/`.clay-card-illustration` CSS + lavender/cream tokens. Phase 2 (chart fixes, `d2a183d`) gave `ClayPillBarChart` per-category colors and `ClayDonutChart` a legend with percentages. Phase 3 (sidebar/tiles/mascot, `711a937`) added the sidebar lavender active-pill + Go-Premium card, expanded the dashboard to a 5-tile pastel grid, and integrated the hero mascot. Phase 4 (typography/QA/deploy, `06c35e5`, EVL-closed 18-07-26) applied `font-cozy` (Quicksand) to a locked element list (hero h1/h2/nav-brand, dashboard h1 + 5 stat-tile numbers, sidebar Go-Premium label), extended `apps/web/e2e/visual-evidence.spec.ts` to a 3-route/12-screenshot matrix (11/12 captured; 1 miss is a foreign missing Supabase RPC), and drafted (never executed) a gayo-vps deploy request. Test baseline moved 37/13 → **48 tests / 15 files** (new `apps/web/app/__tests__/font-cozy-sweep.test.tsx`). 9 of the SPEC's 10 acceptance criteria are fully MET; the 10th (AC9, "existing build/tsc/test gates stay green") is partially unmet on its build/tsc leg — RED across Phases 2-4, but 100% foreign-attributed (0 errors in any program file; see `foreign-build-tsc-red_NOTE_18-07-26.md`), resolution owned by the user's `console-errors-cleanup` general plan. A11y baseline moved 8→6 pre-existing foreign color-contrast fails (net improvement, 0 NEW introduced by this program) — see `process/context/tests/all-tests.md`. **Deploy is NOT executed** — the Phase 4 deploy request is a draft only, gated on explicit user authorization and on re-verifying the gayo-vps deploy path (SSH recon during Phase 4 was flaky; `/home/cozy/htdocs/higherbits.dev` confirmed to still exist, `/home/higherbits/htdocs/higherbits.dev` — the documented live path — unconfirmed this pass). Task folder intentionally kept in `active/` (not archived) pending that user-authorized deploy step — see `process/features/claymorphism-reference-parity/active/claymorphism-reference-parity_16-07-26/claymorphism-reference-parity-umbrella_PLAN_16-07-26.md` `## Program Closeout` for full Definition-of-Done scoring.
