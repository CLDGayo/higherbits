...
- **Claymorphism + 3D Pastel Soft UI program — Phase 1 tokens shipped, declared-only (added 15-07-26).** `apps/web/app/globals.css` now has `--clay-shadow-light/dark/outer`, `--clay-depth-sm/md/lg`, `--clay-pressed`, `--accent-yellow(-foreground)`, and a `.clay-surface` utility (both `:root` and `.dark`), added additively by `process/features/claymorphism-3d-redesign/` Phase 1. None of these tokens are consumed by any route yet — first real consumption starts at that program's Phase 3 (Component Library).
